    ...
    public class WaypointListDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (items.arraySize > 0)
            {
                ...
                {
                    var item = i >= 0 ? items.GetArrayElementAtIndex(i) : null;
                    ...
                    for (int n = 0; n < props.Length; ++n)
                    {
                        ...
                        else
                        {
                            if (n == 0)
                            {
                                ...
                                var waypoint = EditorGUI.ObjectField(rect, item.objectReferenceValue, typeof (Transform), true);
                                if (EditorGUI.EndChangeCheck())
                                {
                                    item.objectReferenceValue = waypoint;
                                }
                            }
                            else
                            {
                                if (GUI.Button(rect, props[n]))
                                {
                                    ...
                                    {
                                        case "-":
                                            int previousSize = items.arraySize;
                                            items.DeleteArrayElementAtIndex(i);
                                            if (items.arraySize == previousSize)
                                            {
                                                items.DeleteArrayElementAtIndex(i);
                                            }
                                            changedLength = true;
