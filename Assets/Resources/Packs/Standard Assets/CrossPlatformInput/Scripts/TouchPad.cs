...
namespace UnityStandardAssets.CrossPlatformInput
{
	...
	public class TouchPad : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
	{
		...
		bool m_Dragging;
		int m_Id = -1;
		Vector2 m_CurrentPointerPosition;
    ...
    private Image m_Image;
		...
		public void OnPointerDown(PointerEventData data)
		{
			...
			m_Dragging = true;
			m_Id = data.pointerId;
			m_CurrentPointerPosition = data.position;
			m_PreviousTouchPos = data.position;
#if !UNITY_EDITOR
        if (controlStyle != ControlStyle.Absolute )
            m_Center = data.position;
#else
			m_PreviousMouse = data.position;
#endif
		}
		...
		void Update()
		{
            ...
            if (controlStyle == ControlStyle.Swipe)
            {
                ...
                m_PreviousTouchPos = pointerPosition;
            }
            Vector2 pointerDelta = new Vector2(pointerPosition.x - m_Center.x , pointerPosition.y - m_Center.y).normalized;
            pointerDelta.x *= Xsensitivity;
            pointerDelta.y *= Ysensitivity;
#else
			Vector2 pointerDelta;
			pointerDelta.x = pointerPosition.x - m_PreviousMouse.x;
			pointerDelta.y = pointerPosition.y - m_PreviousMouse.y;
			m_PreviousMouse = new Vector3(pointerPosition.x, pointerPosition.y, 0f);
#endif
		...
		{
			if (!m_Dragging || data.pointerId != m_Id) return;
			m_CurrentPointerPosition = data.position;
		}

		bool TryGetPointerPosition(out Vector2 position)
		{
			position = m_CurrentPointerPosition;
			return m_Dragging;
		}
		...
		void OnDisable()
		{
			ReleasePointer();
			if (m_UseX && m_HorizontalVirtualAxis != null &&
				ReferenceEquals(CrossPlatformInputManager.VirtualAxisReference(m_HorizontalVirtualAxis.name), m_HorizontalVirtualAxis))
				m_HorizontalVirtualAxis.Remove();

			if (m_UseY && m_VerticalVirtualAxis != null &&
				ReferenceEquals(CrossPlatformInputManager.VirtualAxisReference(m_VerticalVirtualAxis.name), m_VerticalVirtualAxis))
