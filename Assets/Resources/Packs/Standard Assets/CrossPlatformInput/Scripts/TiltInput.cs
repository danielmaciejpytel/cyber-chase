...
namespace UnityStandardAssets.CrossPlatformInput
{
    ...
    public class TiltInput : MonoBehaviour
    {
        ...
        private bool m_AccelerometerWasEnabled;


        private void OnEnable()
        {
            m_Accelerometer = Accelerometer.current;
            if (m_Accelerometer != null)
            {
                m_AccelerometerWasEnabled = m_Accelerometer.enabled;
                if (!m_AccelerometerWasEnabled)
                {
                    InputSystem.EnableDevice(m_Accelerometer);
                }
            }
        ...
        {
            float angle = 0;
            Vector3 acceleration = m_Accelerometer != null
                ? m_Accelerometer.acceleration.ReadValue()
                : Vector3.zero;
            if (acceleration != Vector3.zero)
            {
                switch (tiltAroundAxis)
                {
                    case AxisOptions.ForwardAxis:
                        angle = Mathf.Atan2(acceleration.x, -acceleration.y)*Mathf.Rad2Deg +
                                centreAngleOffset;
                        break;
                    case AxisOptions.SidewaysAxis:
                        angle = Mathf.Atan2(acceleration.z, -acceleration.y)*Mathf.Rad2Deg +
                                centreAngleOffset;
                        break;
                }
            }
        ...
        {
            if (m_Accelerometer != null && !m_AccelerometerWasEnabled && m_Accelerometer.added)
            {
                InputSystem.DisableDevice(m_Accelerometer);
            }
            m_Accelerometer = null;

            if (m_SteerAxis != null &&
                ReferenceEquals(CrossPlatformInputManager.VirtualAxisReference(m_SteerAxis.name), m_SteerAxis))
            {
                m_SteerAxis.Remove();
            }
            m_SteerAxis = null;
