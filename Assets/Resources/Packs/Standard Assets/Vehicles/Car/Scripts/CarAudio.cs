...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAudio : MonoBehaviour
    {
        ...
        {
            // Validate before creating any sources, so an incomplete clip setup cannot leave
            // a partially started engine behind on every retry.
            if (highAccelClip == null ||
                (engineSoundStyle == EngineAudioOptions.FourChannel &&
                 (lowAccelClip == null || lowDecelClip == null || highDecelClip == null)))
            {
                return;
            }
        ...
        {
            // Release only this engine's audio sources.
            DestroyEngineAudioSource(ref m_HighAccel);
            DestroyEngineAudioSource(ref m_LowAccel);
            DestroyEngineAudioSource(ref m_LowDecel);
            DestroyEngineAudioSource(ref m_HighDecel);
            m_StartedSound = false;
        }


        private static void DestroyEngineAudioSource(ref AudioSource source)
        {
            if (source != null)
            {
                Destroy(source);
        ...
        private void Update()
        {
            // CameraSwitch can change the active main camera during play.
